...
func(currentTraining *Training) Train( iters int, rate float32) {
	for i := 0; i < iters; i++ {
		...
		actual := currentTraining.currentPerceptron.FeedForward(point, func(f float32) float64 {
			return activations.SignumFunc(f);
		})
...
func(currentTraining *Training) Verify() int32 {
	...
	for i := 0; i < 100; i++ {
		...
		result := currentTraining.currentPerceptron.FeedForward(point, func(f float32) float64 {
			return activations.SignumFunc(f);
		})
